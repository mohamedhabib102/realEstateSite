...
const Footer: React.FC = () => {
  ...
  const contactInfo: LinkItem[] = [
    {
      icon: <SvgIcon path="/images/phone.svg" alt="Phone" className="w-5 h-5" />,
      text: "+20 0111535882",
      link: "tel:+200111535882",
    },
    {
      icon: <MdOutlineEmail color="#D4A574" className="w-5 h-5" />,
      text: "info@havenix.com",
      link: "mailto:info@havenix.com",
    },
    {
      icon: <SvgIcon path="/images/location.svg" alt="Location" className="w-5 h-5" />,
      text: "123 Nasr City, Egypt",
    },
  ];
      ...
      <div className="container mx-auto lg:px-36 md:px-8 px-3.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 pb-10">
          ...
          <div className="text-center sm:text-left">
            <div className="flex justify-center sm:justify-start items-center text-xl font-bold mb-4 text-white">
              <Logo color="white" />
            </div>
            <p className="text-sm leading-relaxed text-[#D1D5DB] max-w-xs mx-auto sm:mx-0">
          ...
          <div className="text-center sm:text-left">
            ...
            <ul className="space-y-4">
              {contactInfo.map((item, index) => (
                <li key={index} className="flex justify-center sm:justify-start items-center space-x-3 rtl:space-x-reverse">
                  <a
                    href={item.link}
                    className="flex gap-1 items-center text-sm text-[#D1D5DB] hover:text-[#D4A574] transition-colors duration-200"
                  >
                    {item.icon}
                    <span>{item.text}</span>
                  </a>
